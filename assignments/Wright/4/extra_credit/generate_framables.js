#!/usr/bin/env node
// generate_framables.js (updated)
// Usage: node generate_framables.js <sites.txt> <responses_dir> <out_dir>
//
// Changes:
// - Output filenames are prefixed with a numeric index: 01_site.html, 02_site.html, ...
// - Each generated page now includes a "Bypass using local proxy" iframe that points to /proxy?url=ENCODED_TARGET_URL
// - Keeps the saved HTTP response evidence and index table with consistent numbering
...
function sanitizeForFilename(site) {
...
for (let i = 0; i < sites.length; i++) {
  ...
  try {
    ...
    const outName = `${indexNum}_${sanitizeForFilename(site)}.html`;
    const outPath = path.join(outDir, outName);
    const iframeSrc = `https://${site}`;
    const proxyUrl = `/proxy?url=${encodeURIComponent(iframeSrc)}`;
...
<body>
  ...
  <div class="meta">
    <strong>Index:</strong> ${indexNum}<br>
  ...
  <h2>Live attempt to render (original)</h2>
  ...
  <h2>Bypass using local proxy</h2>
  <p>This iframe points to the local <code>/proxy</code> route which fetches and re-serves the target without upstream framing headers:</p>
  <iframe src="${proxyUrl}" title="proxy-bypass-${escapeHtml(site)}"></iframe>
...
// Build a prettier index.html (table + CSS) that includes the numeric index in the table
...
<body>
  <div class="container">
    ...
    <p class="lead">Click a site to open the test page (original iframe + bypass via /proxy + saved response). After opening, inspect DevTools Console/Network for why a site was blocked.</p>
